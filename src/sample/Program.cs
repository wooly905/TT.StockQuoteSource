...
namespace SampleApp
{
    class Program
    {
        ...
        {
            return provider.CreateStockDataSource().FirstOrDefault(a => a.Source == StockQuoteSource.Yahoo);
        }

        static IStockQuoteDataSource GetAlphaVantageDataSource(IStockQuoteProvider provider)
        {
            return provider.CreateStockDataSource().FirstOrDefault(a => a.Source == StockQuoteSource.AlphaVantage);
        }
